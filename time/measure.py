...
def cal_PA_indegree(fn):
	...
	for edge in edges:
		...
		v=int(edge[1])
		if indegree.has_key(u):
			sth[edge]=indegree[u]*indegree[v]
		else:
			sth[edge]=0
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg

def cal_PA_outdegree(fn):
	sth={}
	edges=get_edges(fn)
	indegree,outdegree=get_indegree_outdegree(fn)	
	del indegree
	for edge in edges:
		u=int(edge[0])
		v=int(edge[1])
		if outdegree.has_key(v):
			sth[edge]=outdegree[u]*outdegree[v]
		else:
			sth[edge]=0
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg

def cal_PA_inoutdegree(fn):
	sth={}
	edges=get_edges(fn)
	indegree,outdegree=get_indegree_outdegree(fn)	
	for edge in edges:
		...
		v=int(edge[1])
		if indegree.has_key(u):
			in_sum=indegree[u]+indegree[v]
		else:
			in_sum=indegree[v]
		if outdegree.has_key(v):
			out_sum=outdegree[u]+outdegree[v]
		else:
			out_sum=outdegree[u]
		sth[edge]=in_sum*out_sum
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg	

def cal_PA_follower(fn):
	sth={}
	edges=get_edges(fn)
	follower,followee=get_follower_followee(fn)	
	del followee
	for edge in edges:
		...
		v=int(edge[1])
		if follower.has_key(u):
			sth[edge]=len(follower[u])+len(follower[v])
		else:
			sth[edge]=len(follower[v])
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg

def cal_PA_followee(fn):
	sth={}
	edges=get_edges(fn)
	follower,followee=get_follower_followee(fn)	
	del follower
	for edge in edges:
		u=int(edge[0])
		v=int(edge[1])
		if followee.has_key(v):
			sth[edge]=len(followee[u])+len(followee[v])
		else:
			sth[edge]=len(followee[u])
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg

def cal_PA_follower_followee(fn):
	sth={}
	edges=get_edges(fn)
	follower,followee=get_follower_followee(fn)	
	for edge in edges:
		...
		v=int(edge[1])
		if follower.has_key(u):
			er_sum=len(follower[u])+len(follower[v])
		else:
			er_sum=len(follower[v])
		if followee.has_key(v):
			ee_sum=len(followee[u])+len(followee[v])
		else:
			ee_sum=len(followee[u])
		sth[edge]=er_sum*ee_sum
	sth_avg=get_sth_avg(fn,sth)
	return sth_avg
